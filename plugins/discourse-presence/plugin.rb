...
after_initialize do
  ...
  reloadable_patch do
    TopicViewSerializer.class_eval do
      has_one :reply_presence_state, serializer: PresenceChannelStateSerializer, embed: :objects
      has_one :whisper_presence_state, serializer: PresenceChannelStateSerializer, embed: :objects

      def reply_presence_state
        PresenceChannel.new("/discourse-presence/reply/#{object.topic.id}").state
      end

      def whisper_presence_state
        PresenceChannel.new("/discourse-presence/whisper/#{object.topic.id}").state
      end

      def include_reply_presence_state?
        SiteSetting.presence_enabled
      end

      def include_whisper_presence_state?
        SiteSetting.presence_enabled && scope.is_staff?
      end
    end
  end
end
